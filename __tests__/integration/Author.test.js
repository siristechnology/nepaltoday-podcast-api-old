...
const { createMultipleAuthors, createSingleAuthor } = require('../helpers/author/createAuthor')

const { createMultiplesPodcasts, createSinglePodcast } = require('../helpers/podcast/createPodcast')
...
describe('Testing Author Routes', () => {
	...
	describe('POST in /authors', () => {
		it("should persist an Author and return it's id", async (done) => {
			const { status, body } = await request(app)
				.post('/nepaltoday-podcast/api/v1/authors')
				.send(fakeAuthor)
		...
		it('should not create an Author when missing some required fields and return a 400 HTTP status code and an error message', async (done) => {
			...
			expect(body.message).toBe('Author validation failed: name: Path `name` is required.')
	...
	describe('GET in /authors', () => {
		it("should return an empty array when theres's no authors saved", async (done) => {
			const { status, body } = await request(app).get('/nepaltoday-podcast/api/v1/authors')
	...
	describe('GET in /authors/filter', () => {
		it('should return the authors that matches with the given name', async (done) => {
			...
			const { body, status } = await request(app).get(`/nepaltoday-podcast/api/v1/authors/filter?name=${author.name}`)
		...
		it("should return an empty array when there's no authors saved", async (done) => {
			const { body, status } = await request(app).get('/nepaltoday-podcast/api/v1/authors/filter?name=stenio')
		...
		it("should return a 400 HTTP code with an error message if the filter 'name' is missed", async (done) => {
			...
			const { body, status } = await request(app).get(`/nepaltoday-podcast/api/v1/authors/filter?other=${author.name}`)
	...
	describe('GET in /authors/:id', () => {
		it('should read and return the author with id equal to id received', async (done) => {
			...
			const { status, body } = await request(app).get(`/nepaltoday-podcast/api/v1/authors/${author.id}`)
		...
		it("should return a 404 HTTP code with an error message if there's no author with the id received", async (done) => {
			const { body, status } = await request(app).get('/nepaltoday-podcast/api/v1/authors/123456789987654321123456')
		...
		it("should return a 500 HTTP status code if the id isn't in mongodb default", async (done) => {
			const { status, body } = await request(app)
				.get('/nepaltoday-podcast/api/v1/authors/xyz')
				.send(fakeAuthor)
	...
	describe('PATCH in /authors/:id', () => {
		it('should return 404 HTTP status code with an error message if the author doesn`t exist', async (done) => {
			const { status, body } = await request(app)
				.patch('/nepaltoday-podcast/api/v1/authors/123456789987654321123456')
				.send(fakeAuthor)
		...
		it('should receive an author, update it with new values and return the author updated', async (done) => {
			...
			const { status, body } = await request(app)
				.patch(`/nepaltoday-podcast/api/v1/authors/${author.id}`)
				.send(author)
		...
		it("should return a 500 HTTP status code if the id isn't in mongodb default", async (done) => {
			const { status, body } = await request(app)
				.patch('/nepaltoday-podcast/api/v1/authors/xyz')
				.send(fakeAuthor)
	...
	describe('DELETE in /authors/:id', () => {
		it('should remove the author with the id received', async (done) => {
			...
			const { status, body } = await request(app).delete(`/nepaltoday-podcast/api/v1/authors/${author.id}`)
		...
		it('should return a 500 HTTP status code if the author doesn`t exist', async (done) => {
			const { status, body } = await request(app).delete('/nepaltoday-podcast/api/v1/authors/123456789987654321123456')
		...
		it("should return a 500 HTTP status code if the id isn't in mongodb default", async (done) => {
			const { status, body } = await request(app).delete('/nepaltoday-podcast/api/v1/authors/xyz')
	...
	describe('POST in /authors/:id/podcasts', () => {
		it("should create a Podcast and link with it's Author", async (done) => {
			...
			const { body, status } = await request(app)
				.post(`/nepaltoday-podcast/api/v1/authors/${author.id}/podcasts`)
				...
				.attach('file', `${__dirname.replace(/^(.*\/__tests__)(.*)$/, '$1')}/test.mp3`)
			...
			const isPodcastsLinkedWithAuthor = podcasts.findIndex((podcast) => podcast.id === body.podcast.id)
		...
		it('should not create a Podcast when some required field were forgotten and return a 400 HTTP status code', async (done) => {
			...
			expect(body.message).toBe('Podcast validation failed: stars: Path `stars` is required.')
		...
		it("should return an error message and 404 status code if the author does't exist", async (done) => {
			const { status, body } = await request(app)
				.post('/nepaltoday-podcast/api/v1/authors/123456789987654321123456/podcasts')
				.field('thumbnailImageURL', fakePodcast.thumbnailImageURL)
				.field('description', fakePodcast.description)
				.field('imageURL', fakePodcast.imageURL)
				.field('category', fakePodcast.category)
		...
		it('should return an error message and 400 HTTP status code if the file is missing', async (done) => {
			...
			const { status, body } = await request(app)
				.post(`/nepaltoday-podcast/api/v1/authors/${id}/podcasts`)
				.field('thumbnailImageURL', fakePodcast.thumbnailImageURL)
				.field('description', fakePodcast.description)
				.field('imageURL', fakePodcast.imageURL)
				.field('category', fakePodcast.category)
	...
	describe('GET /podcasts', () => {
		it('should return all podcasts saved', async (done) => {
			...
			const { status, body } = await request(app).get('/nepaltoday-podcast/api/v1/podcasts')
	...
	describe('GET /podcasts/:id', () => {
		it('should return the podcast with the id received', async (done) => {
			...
			const { status, body } = await request(app).get(`/nepaltoday-podcast/api/v1/podcasts/${podcast.id}`)
		...
		it('should return an error message and 500 HTTP status code if the id is out of mongodb pattern', async (done) => {
			const { status, body } = await request(app).get('/nepaltoday-podcast/api/v1/podcasts/not-exist')
		...
		it("should return an error message and 404 HTTP status code if author doesn't exist", async (done) => {
			const { status, body } = await request(app).get('/nepaltoday-podcast/api/v1/podcasts/123456789987654321123456')
	...
	describe('GET /podcasts/:id/download', () => {
		it('should stream the podcast file for download', async (done) => {
			const destination = `${__dirname.replace(/^(.*\/__tests__)(.*)$/, '$1')}/download.mp3`
			...
			const responseStream = request(app)
				.get(`/nepaltoday-podcast/api/v1/podcasts/${body.podcast.id}/download`)
				.parse(binaryParser)
				.buffer()
		...
		it("should return a 500 HTTP status code if the id isn't in mongodb default", async (done) => {
			const { status, body } = await request(app).get('/nepaltoday-podcast/api/v1/podcasts/xyz/download')
		...
		it("should return a 404 HTTP status code with a error message when the podcast doesn't exist", async (done) => {
			const { status, body } = await request(app).get('/nepaltoday-podcast/api/v1/podcasts/123456789987654321123456/download')
	...
	describe('GET /podcasts/:id/listen', () => {
		it('should stream a podcast file', async (done) => {
			...
			const { status, header } = await request(app).get(`/nepaltoday-podcast/api/v1/podcasts/${createPodcastResponse.body.podcast.id}/listen`)
		...
		it("should return a 404 HTTP status code when the Podcast doens't exist", async (done) => {
			const { status, body } = await request(app).get('/nepaltoday-podcast/api/v1/podcasts/123456789987654321123456/listen')
		...
		it("should return a 500 HTTP status code if the id isn't in mongodb default", async (done) => {
			const { status, body } = await request(app).get('/nepaltoday-podcast/api/v1/podcasts/xyz/listen')
	...
	describe('Testing the GET /categories/:id route', () => {
		it('should return the featured podcasts, trending podcasts and the authors related with the category', async (done) => {
			...
			const { status, body } = await request(app).get('/nepaltoday-podcast/api/v1/categories/science')
		...
		it("should return a 400 HTTP status code and an error message when the category doesn't exist", async (done) => {
			const { status, body } = await request(app).get('/nepaltoday-podcast/api/v1/categories/xyz')

			expect(status).toBe(400)
			expect(body).toHaveProperty('message', 'Category must be one of: science,technology,philosofy,business,pop-culture,history.')
	...
	describe('Testing the GET /home route', () => {
		it('should return the hottest podcasts, new releases and trending authors based on the categories inside the query filter', async (done) => {
			...
			const { status, body } = await request(app).get('/nepaltoday-podcast/api/v1/home?categories=science&technology')
		...
		it('should return all podcasts and all authors when the filter is equal to all', async (done) => {
			...
			const { status, body } = await request(app).get('/nepaltoday-podcast/api/v1/home?categories=all')
		...
		it("should return one empty array for hottest podcasts, one fornew releases and one for trending authors if the query doens't match with any record", async (done) => {
			...
			const { status, body } = await request(app).get('/nepaltoday-podcast/api/v1/home?categories=business')
		...
		it("should return a 400 HTTP status code with an error message if the filter 'categories' is missed", async (done) => {
			const { body, status } = await request(app).get('/nepaltoday-podcast/api/v1/home?other=xyz')

			expect(status).toBe(400)
			expect(body).toHaveProperty('message', "The filter 'categories' is required.")
