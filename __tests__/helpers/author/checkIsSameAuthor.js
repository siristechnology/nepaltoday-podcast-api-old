...
const checkHasSameCategories = (firstAuthorCategories, secondAuthorCategories) => {
...
const checkIsSameAuthor = (firstAuthor, secondAuthor) => {
	const keys = Object.keys(firstAuthor).filter(
		(key) => key !== 'categories' && key !== 'relatedAuthors' && key !== 'podcasts' && key !== '__v' && key !== '_id' && key !== 'id',
	)

	const hasSameCategories = checkHasSameCategories(firstAuthor.categories, secondAuthor.categories)
